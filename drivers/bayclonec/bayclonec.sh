...
#SBATCH --output=bayclonec-%j.out
#SBATCH --error=bayclonec-%j.err
#SBATCH --job-name=smchet-bayclonec
...
function usage()
{
        echo "bayclonec.sh"    " [a.k.a. *this* script] "
        echo "Author: Kami E. Chiotti "
        echo "Date: 01.22.20"
        echo
        echo "A wrapper for the SMC-Het Dream Challenge submission, 'subhajit0606/bayclonec'. It "
        echo "builds and launches the command to execute the BayCloneC CWL CommandLineTool within a docker container, "
        echo "then collects and returns the output files as a tarball. "
        echo
        echo "NOTE #1: The 'bayclonec_template.json' template file must be in the same directory as *this* "
        echo "script. "
        ...
        echo " [-c CWL]      - Full path to the CWL tool [e.g., /<full>/<path>/<to>/bayclonec.cwl"
        echo " [-d DRIVERS]  - Full path to *directory* containing shell, json, and submission scripts  [e.g., "
        echo "                 /<full>/<path>/<to>/drivers/bayclonec]. These are the scripts required to run the "
        echo "                 containerized bayclonec algorithm. "
        echo " [-a ALPHA     - Full path to the top directory of the bayclonec file tree, which contains the scripts "
        echo "                 that run the bayclonec algorithm itself. "
        exit
}
...
cd $DRIVERS
TMPJSON=$DRIVERS/bayclonec_template.json
VCF=$TUMOR/*mutect.vcf
CNA=$TUMOR/*battenberg.txt
CELLULARITY=$TUMOR/*cellularity_ploidy.txt
OUTDIR=$DRIVERS/`basename $TUMOR`/bayclonec_outputs
...
WORKDIR=`mktemp -d -p /mnt/scratch/ bayclonec.XXX`
chmod -R 775 $WORKDIR
chmod -R g+s $WORKDIR
JSON=$WORKDIR/bayclonec_template.json
...
tar -czf bayclonec.tar.gz *predfile
rsync -a bayclonec.tar.gz $OUTDIR
